    ...
    public static int lerInt(String texto, int min, int max) {
        ...
        do {
            ...
            if (sc.hasNextInt()) {
                num = sc.nextInt();
                if (num < min || num > max) {
                    System.out.println("Nùmero inválido. Número deve estar compreendido entre " + min + " e " + max + ".");
                }
            } else {
    ...
    public static long lerLong(String texto, long min, long max) {
        ...
        do {
            ...
            if (sc.hasNextLong()) {
                num = sc.nextLong();
                if (num < min || num > max) {
                    System.out.println("Nùmero invalido. Nùmero deve estar compreendido entre " + min + " e " + max + ".");
                }
            } else {
                System.out.println("Tem de introduzir um nùmero inteiro!");
            }
            sc.nextLine();
        } while (num < min || num > max);
        return num;
    ...
    public static float lerFloat(String texto, float min, float max) {
        ...
        do {
            ...
            if (sc.hasNextFloat()) {
                ...
                System.out.println("Tem de introduzir um nùmero real!");
            }
            sc.nextLine();
        } while (num < min || num > max);
        return num;
    ...
    public static char lerChar(String texto, String opcoes) {
        ...
        do {
            ...
            if (opcoes.indexOf(ch) == -1) {
                System.out.println("Opção invalida. Caracter deve ser um dos seguintes: " + opcoes + ".");
            }
            sc.nextLine();
